...
class SyllabusRejiggerTestCase(unittest.TestCase):
    ...
    def test_main_multiple_questions(self, *_):
        ...
        try:
            self.check_converted(xml_src=xml_src, xml_old=xml_old)
        finally:
            shutil.move(xml_old, xml_src)

    def check_converted(self, xml_src: str, xml_old: str):
        old_tree = ET.parse(xml_old)
        old_root = old_tree.getroot()

        # Find syllabus_data nodes
        old_syllabus_data = old_root.findall(".//siteArchive/syllabus/syllabus_data/asset")
        old_encoded_html = old_syllabus_data[0].get("syllabus_body-html")
        ...
        self.assertEqual(original, old_encoded_html)
        self.assertNotEqual(original, encoded_html)

        before_decoded_html = base64.b64decode(old_encoded_html).decode("utf-8")
        after_decoded_html = base64.b64decode(encoded_html).decode("utf-8")
        self.assertIsNotNone(before_decoded_html)
        self.assertIsNotNone(after_decoded_html)

        with open(self.ROOT_DIR + '/test_files/syllabus-archive/old_syllabus.html', 'r') as file:
            old_syllabus_html = file.read()

        with open(self.ROOT_DIR + '/test_files/syllabus-archive/new_syllabus.html', 'r') as file:
            new_syllabus_html = file.read()

        self.assertEqual(old_syllabus_html, before_decoded_html)

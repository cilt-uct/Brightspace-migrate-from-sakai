    ...
    try:
        ...
        if int(size_result) >= 0:
            ...
            if (size_result < max_size):
                logging.info(f"Sakai Resources size for {SITE_ID} is {format_bytes(size_result)}")
            else:
                logout = login_client.service.logout(session_details[0])
                raise SizeExceededError(f"Sakai Resources size in {SITE_ID} of {format_bytes(size_result)} exceeds limit {format_bytes(max_size)}")
